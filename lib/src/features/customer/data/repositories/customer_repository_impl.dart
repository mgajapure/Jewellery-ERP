...
class CustomerRepositoryImpl implements CustomerRepository {
  ...
  Future<Result<List<Customer>>> getCustomers() async {
    try {
      final response = await _apiClient.get(ApiEndpoints.customers);
  ...
  Future<Result<Customer>> getCustomerById(String id) async {
    try {
      ...
      final data = response.data as Map<String, dynamic>;
      final customer = CustomerModel.fromJson(data['data'] as Map<String, dynamic>).toEntity();
  ...
  AppException _mapDioError(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        if (statusCode == 401) {
          return const AuthException(message: 'Session expired. Please login again.');
        }
        if (statusCode == 404) {
